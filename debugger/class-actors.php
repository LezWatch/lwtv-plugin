...
class LWTV_Debugger_Actors {
	...
	public function find_actors_problems( $items = array() ) {
		...
		foreach ( $actors as $actor_id ) {
			...
			if ( ! empty( $check['insta'] ) ) {
				...
				if ( ( new LWTV_Debugger() )->sanitize_social( $check['insta'], 'instagram' ) !== $check['insta'] ) {
					$problems[] = 'Instagram ID is invalid -- ' . $check['insta'];
				} elseif ( ( new LWTV_Debugger() )->validate_imdb( $check['insta'], 'actor' ) ) {
			...
			if ( ! empty( $check['twits'] ) ) {
				if ( ( new LWTV_Debugger() )->sanitize_social( $check['twits'], 'twitter' ) !== $check['twits'] ) {
					$problems[] = 'Twitter ID is invalid -- ' . $check['insta'];
				} elseif ( ( new LWTV_Debugger() )->validate_imdb( $check['twits'], 'actor' ) ) {
	...
	public function find_actors_no_imdb( $items = array() ) {
		...
		foreach ( $actors as $actor_id ) {
			...
			} elseif ( ( new LWTV_Debugger() )->validate_imdb( $imdb, 'actor' ) === false ) {
	...
	public function check_actors_wikidata( $actors = 0, $items = array() ) {
		...
		foreach ( $actors as $actor_id ) {
			...
			if ( isset( $search_body['search']['0']['id'] ) ) {
				...
				$check_wiki = array(
					'birth'     => ( isset( $wiki_actor['claims']['P569'] ) ) ? ( new LWTV_Debugger() )->format_wikidate( $wiki_actor['claims']['P569'][0]['mainsnak']['datavalue']['value']['time'] ) : '',
					'death'     => ( isset( $wiki_actor['claims']['P570'] ) ) ? ( new LWTV_Debugger() )->format_wikidate( $wiki_actor['claims']['P570'][0]['mainsnak']['datavalue']['value']['time'] ) : '',
					'wikipedia' => $wiki_link,
