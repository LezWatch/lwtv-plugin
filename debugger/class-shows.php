...
class LWTV_Debugger_Shows {
	...
	public function find_shows_problems( $items = array() ) {
		...
		if ( ! empty( $items ) ) {
			...
			// Get all the shows
			$the_loop = ( new LWTV_Features_Loops() )->post_type_query( 'post_type_shows' );
	...
	public function find_intersection_problems( $items = array() ) {
		...
		if ( ! empty( $items ) ) {
			...
			$disabled_shows_loop = ( new LWTV_Features_Loops() )->tax_query( 'post_type_shows', 'lez_intersections', 'slug', 'disabilities' );
		...
		foreach ( $shows as $show_id ) {
			...
			$problems = ( new LWTV_Debugger_Characters() )->check_disabled_characters( $show_id );
	...
	public function find_shows_no_imdb( $items = array() ) {
		...
		foreach ( $shows as $show_id ) {
			...
			} elseif ( ( new LWTV_Debugger() )->validate_imdb( $imdb, 'show' ) === false ) {
