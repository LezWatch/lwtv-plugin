...
class LWTV_Debugger_Shows {
	...
	public function find_shows_problems( $items = array() ) {
		...
		if ( ! empty( $items ) ) {
			...
			// Get all the shows
			$the_loop = ( new LWTV_Queery_Post_Type() )->make( 'post_type_shows' );
	...
	public function find_intersection_problems( $items = array() ) {
		...
		if ( ! empty( $items ) ) {
			...
			$disabled_shows_loop = ( new LWTV_Queery_Taxonomy() )->make( 'post_type_shows', 'lez_intersections', 'slug', 'disabilities' );
