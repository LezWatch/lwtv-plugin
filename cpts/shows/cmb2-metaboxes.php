...
class LWTV_Shows_CMB2 {
	...
	public function cmb2_metaboxes() {
		...
		// Field: Air Dates
		$field_airdates = $cmb_mustsee->add_field( array(
			'name'     => 'Air Dates',
		...
		$field_seasons = $cmb_mustsee->add_field( array(
			'name'            => 'Seasons',
		...
		// Field: Show Format
		$field_format = $cmb_mustsee->add_field( array(
			'name'             => 'Format',
			'desc'             => 'Media format.',
			'id'               => $prefix . 'tvtype',
		...
		// Field: IMDb ID
		$field_imdb = $cmb_mustsee->add_field( array(
			'name'       => 'IMDb ID',
			'id'         => $prefix . 'imdb',
			'type'       => 'text',
			'attributes' => array(
		...
		// Field: Show Genre
		$field_genre = $cmb_mustsee->add_field( array(
			'name'              => 'Genre',
			'desc'              => 'Subject matter.',
			'id'                => $prefix . 'tvgenre',
			'taxonomy'          => 'lez_genres',
		...
		$field_intersectional = $cmb_mustsee->add_field( array(
		...
		// Field: Show Stars
		$field_stars = $cmb_mustsee->add_field( array(
			'name'             => 'Show Stars',
		...
		$field_trigger = $cmb_mustsee->add_field( array(
			'name'             => 'Warning?',
		...
		if ( ! is_admin() ) {
			...
			$row1ms       = $grid_mustsee->addRow();
			$row2ms       = $grid_mustsee->addRow();
			$row3ms       = $grid_mustsee->addRow();
			$row4ms       = $grid_mustsee->addRow();
		...
		if ( ! is_admin() ) {
			...
			$grid_mustsee2 = new \Cmb2Grid\Grid\Cmb2Grid( $cmb_mustsee );
			$rowms2        = $grid_mustsee2->addRow();
		...
		if ( ! is_admin() ) {
			...
			$grid_showdets = new \Cmb2Grid\Grid\Cmb2Grid( $cmb_showdetails );
			$row           = $grid_showdets->addRow();
