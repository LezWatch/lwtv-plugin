...
// Icons
let icon_fail = <svg xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 512 512"><path fill="#c0392b" d="M256 48a208 208 0 1 1 0 416 208 208 0 1 1 0-416zm0 464A256 256 0 1 0 256 0a256 256 0 1 0 0 512zM175 175c-9.4 9.4-9.4 24.6 0 33.9l47 47-47 47c-9.4 9.4-9.4 24.6 0 33.9s24.6 9.4 33.9 0l47-47 47 47c9.4 9.4 24.6 9.4 33.9 0s9.4-24.6 0-33.9l-47-47 47-47c9.4-9.4 9.4-24.6 0-33.9s-24.6-9.4-33.9 0l-47 47-47-47c-9.4-9.4-24.6-9.4-33.9 0z"/></svg>;
let icon_success = <svg xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 512 512"><path fill="#5cb85c" d="M256 48a208 208 0 1 1 0 416 208 208 0 1 1 0-416zm0 464A256 256 0 1 0 256 0a256 256 0 1 0 0 512zM369 209c9.4-9.4 9.4-24.6 0-33.9s-24.6-9.4-33.9 0l-111 111-47-47c-9.4-9.4-24.6-9.4-33.9 0s-9.4 24.6 0 33.9l64 64c9.4 9.4 24.6 9.4 33.9 0L369 209z"/></svg>;
...
const PrePublishCheckList = () => {
	...
	const { featuredImage, postExcerpt, postContent } = useSelect((select) => {
		return {
			...
			postExcerpt: select( 'core/editor' ).getEditedPostAttribute( 'excerpt' ),
	...
	useEffect(() => {
		let lockPost = false;
		setPostExcerptMessage( icon_success );
		setFeaturedImageMessage( icon_success );
		...
		if ( postExcerpt === 0 || truncatedPostContent.indexOf(truncatedPostExcerpt) === 0 ) {
			lockPost = true;
			setPostExcerptMessage( icon_fail );
		}

		// Check the featured image
		if (featuredImage === 0) {
			lockPost = true;
			setFeaturedImageMessage( icon_fail );
		}
	...
	return (
		<PluginPrePublishPanel 
			...
			icon={ 'info-outline' }
		>
			<p><b>Custom Excerpt:</b> {postExcerptMessage}</p>
