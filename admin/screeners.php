	...
	function settings_page() {
		...
		<div class="wrap">
			...
			<h2 class="nav-tab-wrapper">
				<a href="?page=screeners" class="nav-tab <?php echo $active_tab == 'intro' ? 'nav-tab-active' : ''; ?>">Introduction</a>
				<a  href="?page=screeners&tab=videos" class="nav-tab <?php echo $active_tab == 'videos' ? 'nav-tab-active' : ''; ?>">Videos</a>
				<a  href="?page=screeners&tab=networks" class="nav-tab <?php echo $active_tab == 'networks' ? 'nav-tab-active' : ''; ?>">Networks</a>
			</h2>
			...
			<tbody>
				<?php
					...
					foreach( $networks as $network => $url ) {
						...
						echo '
						<tr ' . $class . '>
							<td><a href="' . esc_url( $url ) . '" target="_new">' . $network . '</a></td>
							<td>' . esc_url( $url ) . '</td>
						</tr>
						';
						$number++;
			...
			<ul>
				...
				foreach ( $objects['Contents'] as $object ){
					...
					if ( substr( $object['Key'], -1 ) == '/' ) {
						...
						echo '<button type="button" class="btn btn-info" data-toggle="collapse" data-target="#' . strtolower( $name ) . '">' . $name . '</button>';
						echo '<div id="' . strtolower( $name ) . '" class="collapse"><ul>';
						...
						foreach ( $subobjects['Contents'] as $subobject ){
							if ( substr( $subobject['Key'], -1 ) !== '/' ) {
								...
								$size       = self::MakeReadable( $subobject['Size'] );
								echo '<li>&bull; <a href="' . $signed_url->getUri() .'" download>' . $subobject['Key'] . '</a> - ' . $size . '</li>';
							}
						}
						echo '</ul></div>';
					}
