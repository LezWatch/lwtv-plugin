...
/**
 * class LWTV_Rest_API_Stats_JSON
 *
...
class LWTV_Rest_API_Stats_JSON {
	...
	public function get_actors( $format = 'simple', $page = 1 ) {
		...
		switch ( $format ) {
			case 'id':
				...
				break;
			case 'queer-irl':
				$stats_array = ( new LWTV_Statistics() )->generate( 'actors', 'queer-irl', 'array' );
				break;
			case 'gender':
				$stats_array = ( new LWTV_Statistics() )->generate( 'actors', 'actor_gender', 'array' );
				break;
			case 'sexuality':
	...
	public function get_characters( $format = 'simple', $page = 1 ) {
		...
		switch ( $format ) {
			case 'id':
				...
				break;
			case 'cliches':
				$stats_array = ( new LWTV_Statistics() )->generate( 'characters', 'cliches', 'array' );
				break;
			case 'sexuality':
				$stats_array = ( new LWTV_Statistics() )->generate( 'characters', 'sexuality', 'array' );
				break;
			case 'gender':
				$stats_array = ( new LWTV_Statistics() )->generate( 'characters', 'gender', 'array' );
				break;
			case 'romantic':
				$stats_array = ( new LWTV_Statistics() )->generate( 'characters', 'romantic', 'array' );
				break;
			case 'complex':
				$stats_array    = array();
	...
	public function get_death( $format = 'simple' ) {
		...
		switch ( $format ) {
			case 'complex':
				$stats_array = array(
					'shows'     => ( new LWTV_Statistics() )->generate( 'characters', 'dead-shows', 'array' ),
					'sexuality' => ( new LWTV_Statistics() )->generate( 'characters', 'dead-sex', 'array' ),
					'gender'    => ( new LWTV_Statistics() )->generate( 'characters', 'dead-gender', 'array' ),
					// phpcs:ignore
					// Currently roles is not functional.
					//'roles'     => ( new LWTV_Statistics() )->generate( 'characters', 'dead-roles', 'array' ),
				);
				break;
			case 'years':
				$stats_array = ( new LWTV_Statistics() )->generate( 'characters', 'dead-years', 'array' );
				break;
			case 'list':
				$stats_array = ( new LWTV_Statistics() )->generate( 'characters', 'dead-list', 'array' );
				break;
			case 'simple':
	...
	public function get_shows( $format = 'simple', $page = 1 ) {
		...
		switch ( $format ) {
			...
			case 'name':
				...
				break;
			case 'tropes':
				$stats_array = ( new LWTV_Statistics() )->generate( 'shows', 'tropes', 'array' );
				break;
			case 'nations':
				$stats_array = ( new LWTV_Statistics() )->generate( 'shows', 'country', 'array' );
				break;
			case 'genres':
				$stats_array = ( new LWTV_Statistics() )->generate( 'shows', 'genres', 'array' );
				break;
			case 'triggers':
				$stats_array = ( new LWTV_Statistics() )->generate( 'shows', 'triggers', 'array' );
				break;
			case 'formats':
				$stats_array = ( new LWTV_Statistics() )->generate( 'shows', 'formats', 'array' );
				break;
			case 'stars':
				$stats_array = ( new LWTV_Statistics() )->generate( 'shows', 'stars', 'array' );
				break;
			case 'loved':
				$stats_array = ( new LWTV_Statistics() )->generate( 'shows', 'weloveit', 'array' );
				break;
			case 'worth-it':
				$stats_array = ( new LWTV_Statistics() )->generate( 'shows', 'thumbs', 'array' );
				break;
			case 'intersections':
	...
	public function get_show_taxonomy( $type, $format = 'simple', $page = 1 ) {
		...
		foreach ( $taxonomy as $the_tax ) {
			...
			foreach ( $shows_queery as $show_id ) {
				...
				if ( 'complex' === $format ) {
					$return[ $slug ]['onair']           = ( new LWTV_Statistics() )->count_shows( 'onair', $type, $slug );
					$return[ $slug ]['avg_score']       = ( new LWTV_Statistics() )->count_shows( 'score', $type, $slug );
					$return[ $slug ]['avg_onair_score'] = ( new LWTV_Statistics() )->count_shows( 'onairscore', $type, $slug );
				}
